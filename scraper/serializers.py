...
class LinkedInScrapeRequestSerializer(serializers.Serializer):
    ...
    linkedin_url = serializers.URLField(required=True, help_text="LinkedIn profile URL to scrape")
    scraper_id = serializers.IntegerField(required=False, help_text="ID of the Scraper instance to use (optional)")

    def validate_linkedin_url(self, value):
        """Validate that the URL is a LinkedIn profile URL"""
        if 'linkedin.com/in/' not in value:
            raise serializers.ValidationError("Must be a valid LinkedIn profile URL (containing 'linkedin.com/in/')")
        return value
